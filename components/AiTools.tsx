...
const AiTools: React.FC<AiToolsProps> = ({ onNavigateToAcademy }) => {
  const [activeTab, setActiveTab] = useState<'tabs' | 'detect' | 'library' | 'transcribe'>('tabs');
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto h-full animate-in fade-in duration-500">
      <div className="flex justify-center mb-8 gap-4 flex-wrap">
        <button onClick={() => setActiveTab('tabs')} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all ${activeTab === 'tabs' ? 'bg-primary-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
          <FileText className="w-5 h-5" /> Tab Finder
        </button>
        <button onClick={() => setActiveTab('transcribe')} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all ${activeTab === 'transcribe' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
          <FileAudio className="w-5 h-5" /> Audio-to-Tab
        </button>
        <button onClick={() => setActiveTab('detect')} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all ${activeTab === 'detect' ? 'bg-accent-500 text-white' : 'bg-gray-800 text-gray-400'}`}>
          <Music className="w-5 h-5" /> Identify Song
        </button>
        <button onClick={() => setActiveTab('library')} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all ${activeTab === 'library' ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              ...
              {tabResult && (
                 <div className="animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div className="bg-gray-950 p-2 rounded border border-gray-800">
                                        ...
                                        <span className="text-white font-bold">{tabResult.key || 'Unknown'}</span>
                                    </div>
                                    <div className="bg-gray-950 p-2 rounded border border-gray-800">
                                        <span className="text-[10px] text-gray-500 block uppercase">Scale</span>
                                        <span className="text-white font-bold">{tabResult.scale || 'Unknown'}</span>
                                    </div>
                                    <div className="bg-gray-950 p-2 rounded border border-gray-800">
                                        <span className="text-[10px] text-gray-500 block uppercase">Difficulty</span>
                                        <span className={`font-bold ${tabResult.difficulty === 'Advanced' ? 'text-red-400' : 'text-green-400'}`}>{tabResult.difficulty || 'Intermediate'}</span>
                                    </div>
                                    <div className="bg-gray-950 p-2 rounded border border-gray-800">
                                        <span className="text-[10px] text-gray-500 block uppercase">Tuning</span>
                                        <span className="text-white font-bold">{tabResult.tuningName || 'Standard'}</span>
                                    </div>
                                </div>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4 gap-4">
                        <div>
                            <h3 className="text-xl font-bold text-white">{tabResult.song}</h3>
                            <p className="text-gray-400 text-sm">{tabResult.artist}</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 bg-gray-800 p-2 rounded-xl border border-gray-700 shadow-lg w-full md:w-auto">
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <select value={playSpeed} onChange={(e) => setPlaySpeed(Number(e.target.value))} className="bg-transparent text-xs text-white border rounded border-gray-600 px-1 py-0.5">
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <select value={instMode} onChange={(e) => setInstMode(e.target.value as TabInstrumentMode)} className="bg-transparent text-xs text-white border rounded border-gray-600 px-1 py-0.5">
                    ...
                    <div className="relative">
                        <div className="absolute top-0 right-0 bg-gray-800 text-[10px] text-gray-400 px-2 py-1 rounded-bl-lg border-b border-l border-gray-700 font-mono">Original Tab</div>
